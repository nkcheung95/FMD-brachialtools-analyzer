...
dia_data - read.delim(dataFMD_DIA.txt, header=F,sep=,, skip=48)
dia_data - rename(dia_data, diameter= V2, index=V1)
lc_data - read.delim(dataFMD_LC.txt, header=F, sep=t,skip=9)
lc_bl_data - read.delim(dataBL_LC.txt, header=F, sep=t,skip=9)
#visco_data - read.csv(dataVISC.csv, header=F,skip=45)
...
bl_data - read.delim(dataBL_DIA.txt, header=F,sep=,,skip=49)
...
#fmd
fmd_clean$shear_rate - 8(fmd_clean$flow_velfmd_clean$diameter)
fmd_clean$bulk_flow - fmd_clean$flow_vel(pi((fmd_clean$diameter20)^2))60
fmd_clean$fvc - fmd_clean$bulk_flowfmd_clean$fing_pres

#bl
bl_data$shear_rate - 8(bl_data$flow_velbl_data$diameter)
bl_data$bulk_flow - bl_data$flow_vel(pi((bl_data$diameter20)^2))60
bl_data$fvc - bl_data$bulk_flowbl_data$fing_pres
...
fmd_per - fmd_mmbl_diameter100
fmd_mm_ss - fmd_mmauc_ss
fmd_per_ss - fmd_perauc_ss
...
img1 - image_read(plotsDIA_BL.png)
img2 - image_read(plotsSR_BL.png)
img3 - image_read(plotsFV_BL.png)

img5 - image_read(plotsDIA_PO.png)
img6 - image_read(plotsSR_PO.png)
img7 - image_read(plotsFV_PO.png)
...
if (file.exists(Master FMD ExportMASTER_FMD_DATA.csv) ){
  
  write.table( results,  
               file=Master FMD ExportMASTER_FMD_DATA.csv, 
               append = T, 
               sep=',', 
               row.names=F, 
               col.names=F )
